...
const MediaPlayerRoot = React.forwardRef<HTMLDivElement, MediaPlayerRootProps>(
  (props, forwardedRef) => {
    ...
    const onKeyDown = React.useCallback(
      ...
      [store, propsRef.current.onKeyDown, propsRef.current.onPipError, disabled]
    )
        ...
        <MediaPlayerContext.Provider value={contextValue}>
          <RootPrimitive
            ...
            className={cn(
              ...
              "[&_[data-slider]::before]:absolute [&_[data-slider]::before]:inset-x-0 [&_[data-slider]::before]:-bottom-2 [&_[data-slider]::before]:-top-6 [&_[data-slider]::before]:z-10 [&_[data-slider]::before]:h-12 [&_[data-slider]::before]:cursor-pointer [&_[data-slider]::before]:content-[''] [&_[data-slider]]:relative",
              className
            )}
          >
...
>((props, forwardedRef) => {
  ...
  return (
    <ControlsPrimitive
      ...
      ref={forwardedRef}
      className={cn(
        'dark absolute bottom-0 left-0 right-0 z-50 flex items-center gap-2 px-4 py-3',
        '[:fullscreen_&]:absolute [:fullscreen_&]:bottom-0 [:fullscreen_&]:left-0 [:fullscreen_&]:right-0 [:fullscreen_&]:z-50 [:fullscreen_&]:px-6 [:fullscreen_&]:py-4',
        className
      )}
      {...controlsProps}
    />
        ...
        <TooltipTrigger asChild>
          ...
          >
            ...
            <SliderPrimitive.Thumb
              ...
              className="focus-visible:outline-hidden relative z-10 block size-2.5 shrink-0 rounded-full bg-primary shadow-sm ring-ring/50 transition-[color,box-shadow] hover:ring-4 focus-visible:ring-4 disabled:pointer-events-none disabled:opacity-50"
            />
          </SliderPrimitive.Root>
        </TooltipTrigger>
        {duration > 0 && (
    ...
    >
      ...
      >
        ...
        <SliderPrimitive.Thumb
          ...
          className="focus-visible:outline-hidden block size-2.5 shrink-0 rounded-full bg-primary shadow-sm ring-ring/50 transition-[color,box-shadow] hover:ring-4 focus-visible:ring-4 disabled:pointer-events-none disabled:opacity-50"
        />
      </SliderPrimitive.Root>
    </div>
  )
    ...
    >
      <MediaPlayerTooltip tooltip="Playback speed" shortcut={['<', '>']}>
        <SelectTrigger
          ...
          ref={forwardedRef}
          className={cn(
            'h-8 w-16 justify-center border-none aria-expanded:bg-accent aria-[expanded=true]:bg-accent/50 dark:bg-transparent dark:hover:bg-accent/50 dark:aria-[expanded=true]:bg-accent/50 [&[data-size]]:h-8 [&_svg]:hidden',
            className
          )}
        >
        ...
        {Array.isArray(shortcut) ? (
          <div className="flex items-center gap-1">
            {shortcut.map((shortcutKey) => (
              <kbd
                key={shortcutKey}
                className="shadow-xs select-none rounded border bg-secondary px-1.5 py-0.5 font-mono text-[0.7rem] text-foreground"
              >
          ...
          <kbd
            key={shortcut}
            className="shadow-xs select-none rounded border bg-secondary px-1.5 py-px font-mono text-[0.7rem] text-foreground"
          >
